...
grey = np.array([.5,.5,.5,1])
colors = np.vstack((darkblue,skyblue,white,grey))
x = np.linspace(-1,1,100)
cmap = np.vstack([np.interp(x,np.array([-1, 0, .2, 1]),colors[:,c]) for c in range(4)]).T
    ...
    while True:
        ...
        if tnow-tlastfig>np.timedelta64(2,'m'):
            ...
            if True:
                ...
                plt.pcolormesh(range(Nbuffer), z, V, cmap=cmap, vmin=-15000, vmax=15000, shading="auto")
                for level in output["cloud_base"]:
                    if level<3000:
                        hlvl = plt.annotate(f"  {level:.0f} m", (Nbuffer+1, level), 
                                fontweight='bold',
                                va="center", ha="left",
                                bbox=dict(boxstyle="square,pad=0.1", fc="white", ec="white", lw=0,alpha=0.9)
                                )
                ...
                plt.text(0.5, 0.99, f"EastGRIP Ceilometer {stnow}", 
                        horizontalalignment='center', verticalalignment='top', 
                        fontsize ='large', fontweight='bold', alpha=0.5,
